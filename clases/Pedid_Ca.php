...
class Pedid_Ca{
    ...
    public function editarPedido($conexion){
        ...
        try{
            ...
            $query->bindParam(':Total_Gravado', $Total_Gravado,PDO::PARAM_STR);
            $query->bindParam(':Total_Exento', $Total_Exento,PDO::PARAM_STR);
            $query->bindParam(':Total_Producto_Ex',$Total_Producto_Ex,PDO::PARAM_STR);
            $query->bindParam(':Porc_Iva_Ins',$Porc_Iva_Ins,PDO::PARAM_STR);
            $query->bindParam(':Iva_Ins',$Iva_Ins,PDO::PARAM_STR);
            $query->bindParam(':Porc_Iva_NoIns',$Porc_Iva_NoIns,PDO::PARAM_STR);
            $query->bindParam(':Iva_NoIns',$Iva_NoIns,PDO::PARAM_STR);
    ...
    private function cargarDatosFormulario(){
        if(isset($_POST)){
            ...
            if(isset($pedido['Nro_Pedido']) && $pedido['Nro_Pedido'] > 0)
                $this->Nro_Pedido = intval($pedido['Nro_Pedido']);
            else
                $this->Nro_Pedido = 0;

            $cliente = $pedido['Cliente'];
            if($cliente)
                $this->id_Cliente = intval($cliente['id']);
            else
                $this->id_Cliente = intval($pedido['id_Cliente']);

            $this->Codigo_Vendedor = intval($pedido['Codigo_Vendedor']);
            $this->Nro_Cotizacion = intval($pedido['Nro_Cotizacion']);
            $this->Nro_Presupuesto = intval($pedido['Nro_Presupuesto']);
            $this->Id_Lista = intval($pedido['Id_Lista']);
            $this->Id_Condicion = intval($pedido['Id_Condicion']);
            $this->Id_Moneda = intval($pedido['id_Moneda']);
            $this->Cotizacion_Moneda = floatval($pedido['Cotizacion_Moneda']);
            $this->Estado = $pedido['Estado'];
            $this->Id_Reparto = intval($pedido['Id_Reparto']);
            $this->Nro_orden_compra = intval($pedido['Nro_Orden_Compra']);
            $this->Id_Tomado_Por = intval($pedido['Id_Tomado_Por']);
            $this->Id_Usuario = intval($pedido['id_Usuario']);
            $this->Fecha_Operacion = $pedido['Fecha_Operacion'];
            $this->Fecha_Pedido = $pedido['Fecha_Pedido'];
            $this->Id_Grupo_Cliente = intval($pedido['Id_Grupo_Cliente']);
            $this->Total_Gravado = floatval($pedido['Total_Gravado']);
            $this->Total_Exento = floatval($pedido['Total_Exento']);
            $this->Total_Producto_Ex = floatval($pedido['Total_Producto_Ex']);
            $this->Porc_Iva_Ins = floatval($pedido['Porc_Iva_Ins']);
            $this->Iva_Ins = floatval($pedido['Iva_Ins']);
            $this->Porc_Iva_NoIns = floatval($pedido['Porc_Iva_NoIns']);
            $this->Iva_NoIns = floatval($pedido['Iva_NoIns']);
            $this->Porc_IngBr_Cba = floatval($pedido['Porc_IngBr_Cba']);
            $this->IngBr_Cba = floatval($pedido['IngBr_Cba']);
            $this->Porc_IngBr_Pba = floatval($pedido['Porc_IngBr_Pba']);
            $this->IngBr_Pba = floatval($pedido['IngBr_Pba']);
            $this->Total_Neto = floatval($pedido['Total_Neto']);
            $this->Descuento = floatval($pedido['Descuento']);
            $this->Id_Transporte = intval($pedido['Id_Transporte']);
            $this->Id_Condicion = intval($pedido['Id_Condicion']);
            $this->Generado_Por = intval($pedido['Generado_Por']);
            $this->Porc_IngBr_Mis = floatval($pedido['Porc_IngBr_Mis']);
            $this->IngBr_Mis = floatval($pedido['IngBr_Mis']);
        }
    }
}
